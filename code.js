...
$(document).ready(function(){
	...
	var heroHTML = "<option value=-1 class=\"hero_option\">英雄選択</option>";
	for(var i = 0; i < data.heroes.length; i++){
		heroHTML += "<option value=" + i + " class=\"hero_option\">" + data.heroes[i].name + "</option>";
	}

	//Initiate hero lists
	var listHTML = "<option value=-1 class=\"hero_option\">新規リスト</option>";
...
function getExportText(side){
	...
	function getHeroExportText(hero){
		...
		if(hero.index != -1){
			...
			if(hero.bless_1 != "none" || hero.bless_2 != "none" || hero.bless_2 != "none"){
...
function activeHero(hero){
	...
	this.threaten = function(enemy){
		...
		if(enemy.range == "melee" || options.ployBehavior != "斜め"){
			...
			if(this.hasAtRefineIndex("セルジュの恐斧・専用", this.refineIndex) && this.hp > enemy.hp){
				enemy.panicked = true;
...
//					statChanges.push(stat + " " + debuffVal[stat]);
	...
	this.setCombatSpur = function(enemy){
		...
		if (this.hasExactly("ゲイボルグ") && enemy.moveType != "flying"){
			statBonus = 5;
			this.combatSpur.atk += statBonus;
			this.combatSpur.def += statBonus;
			skillName = data.skills[this.weaponIndex].name;
			boostText += this.name + " は、" + skillName + " の効果で、飛行以外の敵に対して、戦闘中、攻撃、守備 +" + statBonus + " 。<br>";
		}
		if(this.didAttack && this.hasExactly("ライナロック") && this.refineIndex != -1){
			statBonus = 5;
			this.combatSpur.atk += statBonus;
		...
		if ((enemy.hasExactly("夢幻の書") || enemy.hasExactly("泡影の書"))
			&& (enemy.adjacent > 0)
			){
			...
			boostText += this.name + " は、" + data.skills[enemy.weaponIndex].name + " の効果で、戦闘中、攻撃、速さ、守備、魔防 -4 。<br>";
		}
		...
		if (this.combatStartHp / this.maxHp <= 0.8){
			...
			if (this.hasAtIndex("攻撃魔防の大覚醒", this.aIndex)){
				statBonus = 1 + 2 * this.hasAtIndex("攻撃魔防の大覚醒", this.aIndex);
				this.combatSpur.atk += statBonus;
				this.combatSpur.res += statBonus;
				skillName = data.skills[this.aIndex].name;
				boostText += this.name + " は、" + skillName + " の効果で、攻撃、魔防 +" + statBonus + " 。<br>";
			}
			if (this.hasAtIndex("攻撃魔防の大覚醒", this.sIndex)){
				statBonus = 1 + 2 * this.hasAtIndex("攻撃魔防の大覚醒", this.sIndex);
				this.combatSpur.atk += statBonus;
				this.combatSpur.res += statBonus;
				skillName = data.skills[this.sIndex].name;
				boostText += this.name + " は、" + skillName + "(聖印) の効果で、攻撃、魔防 +" + statBonus + " 。<br>";
			}
		...
		if(this.combatStartHp / this.maxHp >= 1){
			if(this.hasExactly("ライナロック") && this.refineIndex == -1){
		...
		if (this.adjacent2 > 0){
			...
			if (this.hasExactly("水の飛沫") && this.adjacent2 > this.adjacent2_foe){
				buffVal = 5;
				skillName = data.skills[this.weaponIndex].name;
				this.combatSpur.atk += buffVal;
				this.combatSpur.spd += buffVal;
		...
		if(!this.initiator){
			...
			if (this.hasExactly("バリアの剣")){
				buffVal = 4
				this.combatSpur.res += buffVal;
				boostText += this.name + " は、" + data.skills[this.weaponIndex].name + " の効果で、敵から攻撃された時、魔防 +" + buffVal +" 。<br>";
			}
			if (this.hasExactly("バリアの剣+")){
				buffVal = 7
				this.combatSpur.res += buffVal;
...
//				statChanges.push("+" + this.combatBuffs[stat] + " " + stat);
		...
		if (this.triggered){
			...
			if (this.hasExactly("魔剣ミストルティン")){
				...
				postCombatBuffText += this.name + " は、" + data.skills[this.weaponIndex].name + " の効果で戦闘後、奥義カウント -" + gainValue + " 。<br>";
			}
			if (this.has("奥義の螺旋")){
				if (this.has("奥義の螺旋") != 1){
					gainValue = this.has("奥義の螺旋") - 1;
				}else if (this.challenger){
					gainValue = 1;
				}
				postCombatBuffText += this.name + " は、" + data.skills[this.bIndex].name + " の効果で戦闘後、奥義カウント -" + gainValue + " 。<br>";
			}
		}
	...
	this.attack = function(enemy, round, buffRound, debuffRound, galeforce){
		...
		*/
		...
		if (this.has("フォルセティ") && this.hp/this.maxHp >= .5){
			desperation = true;
		}
		...
		if (this.has("見切り・追撃効果")){
			if (this.has("見切り・追撃効果") < 3){
				if (this.hp >= this.maxHp/this.has("見切り・追撃効果")){
					enemyRankSkillNull = true;
					roundText += enemy.name + " は、絶対追撃を無効化される。<br>";
				}
			}else{
				enemyRankSkillNull = true;
				roundText += enemy.name + " は、絶対追撃を無効化される。<br>";
		...
		if (enemy.has("見切り・追撃効果")){
			if (enemy.has("見切り・追撃効果") < 3){
				if (enemy.hp >= enemy.maxHp/enemy.has("見切り・追撃効果")){
					thisRankSkillNull = true;
					roundText += this.name + " は、絶対追撃を無効化される。<br>";
				}
			}else{
				thisRankSkillNull = true;
				roundText += this.name + " は、絶対追撃を無効化される。<br>";
		...
		if (!thisRankSkillNull){
			...
			if (this.hasExactly("精霊のブレス") && this.combatStat.def >= enemy.combatStat.def + 5){
				thisAttackRank++;
				thisAttackRankChanged = true;
			}
